...
void
MainWindow::on_actionWaterfall_triggered()
{
    if (!waterfallDlg) {
        waterfallDlg = new Waterfall(this);
        waterfallDlg->setAttribute(Qt::WA_DeleteOnClose);
        connect(waterfallDlg, &QObject::destroyed, this, [this] { waterfallDlg = nullptr; });
    }
    waterfallDlg->show();
    waterfallDlg->raise();
    waterfallDlg->activateWindow();
}
